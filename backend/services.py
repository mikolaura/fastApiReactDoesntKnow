...
async def authencticate_user(email: str, passoword: str, db: _orm.Session):
    user = await get_user_by_email(email, db)
    if not user:
        return False

    if not user.verify_password(passoword):
        return False
...
async def update_lead(
    lead_id: int, user: _schemas.User, db: _orm.Session, lead: _schemas.LeadCreate
):
